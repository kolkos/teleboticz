...
# test insert
query_insert = "INSERT INTO telegram_callback_queries (user_id, query_id, data) VALUES (?, ?, ?)"
...
query_update = "UPDATE telegram_callback_queries SET query_id = ? WHERE user_id = ?"
values_update = [(new_query_id, user_id_2)]
db.update_handler(query_update, values_update)

# test delete
query_delete = "DELETE FROM telegram_callback_queries WHERE query_id = ?"
values_delete = [(query_id_1,)]
db.update_handler(query_delete, values_delete)

# test insert
query_select = "SELECT * FROM telegram_callback_queries WHERE user_id = ? AND date_handled = ?"
...
for row in results:
    ...
    query_handle = "UPDATE telegram_callback_queries SET date_handled = ? WHERE id = ?"
